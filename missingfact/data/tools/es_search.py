...
class EsSearch:
    def __init__(self,
        ...
        """
        self._es = Elasticsearch([es_client], retries=3, timeout=60)
    ...
    def get_hits_for_question(self, question: str, choices: List[str],
                              optional_terms: str=None) -> Dict[str, List[EsHit]]:
        """
    ...
    def construct_qa_query(self, question, choice, optional_terms=None):
        if optional_terms is not None:
            query = question[-self._max_question_length:] + " " + choice + " " + optional_terms
        else:
    ...
    def get_hits_for_choice(self, question, choice, optional_terms=None):
